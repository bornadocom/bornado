...
/**
 * Lightweight vendored copy of the official Hashids PHP implementation.
 *
 * Source project: hashids/hashids
 */
class Hashids implements HashidsInterface {
	...
	protected $shuffledAlphabets = array();
	...
	public function __construct(
		...
		$this->salt          = mb_convert_encoding( $salt, 'UTF-8', mb_detect_encoding( $salt ) ?: 'UTF-8' );
		$this->minHashLength = $minHashLength;
		$alphabet            = mb_convert_encoding( $alphabet, 'UTF-8', mb_detect_encoding( $alphabet ) ?: 'UTF-8' );
	...
	public function encode( ...$numbers ): string {
		...
		foreach ( $numbers as $i => $number ) {
			$alphabet = $this->shuffle(
				$alphabet,
				mb_substr( $lottery . $this->salt . $alphabet, 0, mb_strlen( $alphabet ) )
			);
			$ret .= $last = $this->hash( (string) $number, $alphabet );
	...
	public function decode( string $hash ): array {
		...
		$index         = ( 3 === count( $hashArray ) || 2 === count( $hashArray ) ) ? 1 : 0;
		$hashBreakdown = $hashArray[ $index ];
	...
	public function decodeHex( string $hash ): string {
		...
		foreach ( $numbers as $number ) {
			$ret .= mb_substr( dechex( (int) $number ), 1 );
		}
	...
	protected function multiByteSplit( string $string ): array {
		return preg_split( '/(?!^)(?=.)/u', $string ) ?: array();
	}
}
