jQuery(document).ready(function ($) {
        ...
        await $.ajax({
            url: ajaxurl, type: 'POST', data: {
                ...
                if (response.success) {
                    ...
                    // AI Intent Fields: Populate in Categories tab when AI is enabled
                    if (response.data.ai_intent_in_category) {
                        var hasIntentFields = false;

                        // Populate Ad Type in Categories tab
                        ...
                        // Populate Condition & Warranty in Categories tab
                        ...
                        // Show/hide the intent fields container based on whether we have content
                        if (hasIntentFields) {
                        ...
                        // Apply radio button styling to intent fields
                        setTimeout(function () {
                            applyRadioButtonStyling();
                        }, 100);
                    } else {
                        // AI not enabled - hide the container
                        $("#ai-intent-fields-container").hide();
                    }
                    ...
                    if (response.data.no_template_selected && response.data.default_template_on) {
                        ...
                        if (response.data.more_options) {
                            ...
                            // Only populate legacy condition/warranty box if AI intent fields are NOT in category tab
                            // When AI is enabled, fields go to the category tab containers
    ...
    if ($("#sb_bid_ad").length > 0) {
        ...
        $("#sb_bid_ad").on("submit", function (event) {
            event.preventDefault();
            let form = $(this);
            // Paid-bidding fallback: if the server says paid mode is on and the
            // signed-in user has no credits, redirect to the packages page
            // instead of submitting an AJAX that will only return an error.
            let paidBidding = form.data("paid-bidding");
            let userCredits = String(form.data("user-credits") || "");
            let packageUrl  = form.data("package-url") || "";
            if ((paidBidding === 1 || paidBidding === "1") &&
    ...
    $(document).on('change', 'input.ads-package-radio', function () {
        ...
        try {
            ...
            // Bidding visibility is gated by BOTH the package's allow_bidding
            // flag AND the selected category's adforest_make_bid_cat_base
            // (Selective mode). A permissive package must not override a
            // bid-disabled category — that was the leak that made bidding
            // appear on package-posting even when the category was unchecked.
            if (pkgData.allow_bidding == 0 || pkgData.allow_bidding == "") {
                $('.bidding-content').addClass('collapsed').hide();
                $('#biddingSection').removeClass('show').hide();
            } else if (typeof window.adfRecheckBidding === 'function') {
                window.adfRecheckBidding();
            } else {
    ...
    (function () {
        ...
        // Walk root + every subcategory select and return the DEEPEST selected
        // term id. AdForest categories are hierarchical, and bidding is allowed
        // per leaf term; checking only the root misses the case where a
        // bid-enabled parent has a bid-disabled child (or vice versa).
        function adfGetDeepestCatId() {
            var catId = $catSelect.val() || 0;
            $('.child-category-select').each(function () {
                if ($(this).val()) {
                    catId = $(this).val();
                }
        ...
        function adfCheckBiddingCat(catId) {
            ...
            $.post({
                ...
                // Bidding trigger + panel. Both live in ad_post_shortcode.php (.bidding-content + #biddingSection).
                // Keep Bootstrap collapse state in sync with the display toggle:
                // panel gets the `show` class when visible; trigger gets `collapsed`
                // when the panel is hidden. Without this, Bootstrap and the inline
                // style can disagree ("show + display:none" or vice versa) and the
                // section ends up visible-but-dead.
                var $trigger = $('.bidding-content');
                var $panel   = $('#biddingSection');
                if (response && response.show) {
                    $trigger.removeClass('collapsed').show();
                    $panel.addClass('show').show();
                } else {
                    $trigger.addClass('collapsed').hide();
                    $panel.removeClass('show').hide();
                }
            });
        }

        // PHP owns the initial render gate (see ad_post_shortcode.php). These
        // listeners re-run the server-side check on every category transition
        // — root OR subcategory — against the deepest selected term so the
        // bidding section stays in sync in both free and package flows.
        var initial = adfGetDeepestCatId();
        if (initial) {
            adfCheckBiddingCat(initial);
        }

        $catSelect.on('change', function () {
            // Subcategory dropdowns are injected async; re-check immediately
            // against the deepest value currently present.
            adfCheckBiddingCat(adfGetDeepestCatId());
        });

        // Delegated listener so it catches `.child-category-select` elements
        // that are added to the DOM after this IIFE runs.
        $(document).on('change', '.child-category-select', function () {
            adfCheckBiddingCat(adfGetDeepestCatId());
        });

        // Exposed so other code paths (package selection in paid flow, etc.)
        // can re-apply the category gate without duplicating its logic.
        window.adfRecheckBidding = function () {
            adfCheckBiddingCat(adfGetDeepestCatId());
        };
    })();
