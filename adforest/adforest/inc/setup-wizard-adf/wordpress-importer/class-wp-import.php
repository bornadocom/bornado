...
class WP_Import extends WP_Importer {
	...
	public function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			echo '<p><strong>' . __( 'Sorry, there has been an error.', 'adforest' ) . '</strong><br />';
			echo __( 'The file does not exist, please try again.', 'adforest' ) . '</p>';
			$this->footer();
			die();
		}
	...
	public function import_end() {
		...
		echo '<p>' . __( 'All done.', 'adforest' ) . ' <a href="' . admin_url() . '">' . __( 'Have fun!', 'adforest' ) . '</a>' . '</p>';
		echo '<p>' . __( 'Remember to update the passwords and roles of imported users.', 'adforest' ) . '</p>';
	...
	public function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			echo '<p><strong>' . __( 'Sorry, there has been an error.', 'adforest' ) . '</strong><br />';
			printf( __( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'adforest' ), esc_html( $file['file'] ) );
			echo '</p>';
			return false;
		}
		...
		if ( $this->version > $this->max_wxr_version ) {
			...
			printf( __( 'This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.', 'adforest' ), esc_html( $import_data['version'] ) );
			echo '</strong></p></div>';
		}
	...
	public function get_authors_from_import( $import_data ) {
		if ( ! empty( $import_data['authors'] ) ) {
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					printf( __( 'Failed to import author %s. Their posts will be attributed to the current user.', 'adforest' ), esc_html( $post['post_author'] ) );
					echo '<br />';
					continue;
				}
...
<?php if ( ! empty( $this->authors ) ) : ?>
	<h3><?php _e( 'Assign Authors', 'adforest' ); ?></h3>
	<p><?php _e( 'To make it simpler for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site, such as your primary administrator account.', 'adforest' ); ?></p>
<?php if ( $this->allow_create_users() ) : ?>
	<p><?php printf( __( 'If a new user is created by WordPress, a new password will be randomly generated and the new user&#8217;s role will be set as %s. Manually changing the new user&#8217;s details will be necessary.', 'adforest' ), esc_html( get_option( 'default_role' ) ) ); ?></p>
<?php endif; ?>
	<ol id="authors">
...
<?php if ( $this->allow_fetch_attachments() ) : ?>
	<h3><?php _e( 'Import Attachments', 'adforest' ); ?></h3>
	<p>
		<input type="checkbox" value="1" name="fetch_attachments" id="import-attachments" />
		<label for="import-attachments"><?php _e( 'Download and import file attachments', 'adforest' ); ?></label>
	</p>
<?php endif; ?>

	<p class="submit"><input type="submit" class="button" value="<?php esc_attr_e( 'Submit', 'adforest' ); ?>" /></p>
</form>
		<?php
	...
	public function author_select( $n, $author ) {
		...
		if ( $create_users ) {
			...
			if ( '1.0' != $this->version ) {
				_e( 'or create new user with login name:', 'adforest' );
				$value = '';
			} else {
				_e( 'as a new user:', 'adforest' );
		...
		if ( ! $create_users && '1.0' == $this->version ) {
			_e( 'assign posts to an existing user:', 'adforest' );
		} else {
			_e( 'or assign posts to an existing user:', 'adforest' );
		}
		echo '</label>';
		...
		echo ' ' . wp_dropdown_users(
			array(
				...
				'multi'           => true,
				'show_option_all' => __( '- Select -', 'adforest' ),
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][ $i ] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'adforest' ), esc_html( $this->authors[ $old_login ]['author_display_name'] ) );
	...
	public function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) && $id > 0 ) {
				...
				printf( __( 'Failed to import category %s', 'adforest' ), esc_html( $cat['category_nicename'] ) );
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
	...
	public function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( __( 'Failed to import post tag %s', 'adforest' ), esc_html( $tag['tag_name'] ) );
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
					echo ': ' . $id->get_error_message();
				}
				echo '<br />';
	...
	public function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( __( 'Failed to import %1$s %2$s', 'adforest' ), esc_html( $term['term_taxonomy'] ), esc_html( $term['term_name'] ) );
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
					echo ': ' . $id->get_error_message();
				}
				echo '<br />';
	...
	public function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				printf(
					__( 'Failed to import &#8220;%1$s&#8221;: Invalid post type %2$s', 'adforest' ),
			...
			*/
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				printf( __( '%1$s &#8220;%2$s&#8221; already exists.', 'adforest' ), $post_type_object->labels->singular_name, esc_html( $post['post_title'] ) );
				echo '<br />';
				...
				if ( is_wp_error( $post_id ) ) {
					printf(
						__( 'Failed to import %1$s &#8220;%2$s&#8221;', 'adforest' ),
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							printf( __( 'Failed to import %1$s %2$s', 'adforest' ), esc_html( $taxonomy ), esc_html( $term['name'] ) );
							if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
	...
	public function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			_e( 'Menu item skipped due to missing menu slug', 'adforest' );
			echo '<br />';
			return;
		}
		...
		if ( ! $menu_id ) {
			printf( __( 'Menu item skipped due to invalid menu slug: %s', 'adforest' ), esc_html( $menu_slug ) );
			echo '<br />';
			return;
		} else {
	...
	public function process_attachment( $post, $url ) {
		if ( ! $this->fetch_attachments ) {
			return new WP_Error(
				...
				__( 'Fetching attachments is not enabled', 'adforest' )
			);
		}
		...
		if ( $info ) {
			...
			return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'adforest' ) );
		}
	...
	public function fetch_remote_file( $url, $post ) {
		...
		if ( ! $tmp_file_name ) {
			return new WP_Error( 'import_no_file', __( 'Could not create temporary file.', 'adforest' ) );
		}
		...
		if ( is_wp_error( $remote_response ) ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Request failed due to an error: %1$s (%2$s)', 'adforest' ),
		...
		if ( 200 !== $remote_response_code ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Remote server returned the following unexpected result: %1$s (%2$s)', 'adforest' ),
		...
		if ( ! $headers ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', __( 'Remote server did not respond', 'adforest' ) );
		}
		...
		if ( 0 === $filesize ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'adforest' ) );
		}

		if ( ! isset( $headers['content-encoding'] ) && isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', __( 'Downloaded file has incorrect size', 'adforest' ) );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', sprintf( __( 'Remote file is too large, limit is %s', 'adforest' ), size_format( $max_size ) ) );
		}
		...
		if ( ( ! $type || ! $ext ) && ! current_user_can( 'unfiltered_upload' ) ) {
			return new WP_Error( 'import_file_error', __( 'Sorry, this file type is not permitted for security reasons.', 'adforest' ) );
		}
		...
		if ( ! $move_new_file ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', __( 'The uploaded file could not be moved', 'adforest' ) );
		}
	...
	public function header() {
		echo '<div class="wrap">';
		echo '<h2>' . __( 'Import WordPress', 'adforest' ) . '</h2>';
		...
		if ( isset( $updates[ $basename ] ) ) {
			...
			echo '<div class="error"><p><strong>';
			printf( __( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'adforest' ), $update->update->new_version );
			echo '</strong></p></div>';
		}
	}
	...
	public function greet() {
		...
		echo '<p>' . __( 'Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'adforest' ) . '</p>';
		echo '<p>' . __( 'Choose a WXR (.xml) file to upload, then click Upload file and import.', 'adforest' ) . '</p>';
