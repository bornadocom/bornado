...
if (!function_exists('adforest_get_static_string_fun')) {
    function adforest_get_static_string_fun()
    {
        ...
        return array(
            ...
            "nominatim_proxy_nonce" => wp_create_nonce('adforest_nominatim_proxy_nonce'),
        );
    }
}
                        ...
                        <div class="adt-category-ad-list">
                            ...
                            <div class="category-content-box">
                                <a href="javascript:void(0);"
                                   ...
                                   data-toggle="tooltip"
                                   data-placement="top"
                                   title="<?php echo esc_attr__("Click to make it favourite", "adforest"); ?>">
                ...
                while ($query->have_posts()) {
                    ...
                    if (isset($adforest_theme['turn_on_grid_adverts_search']) && $adforest_theme['turn_on_grid_adverts_search'] == '1') {
                        if ($listing_counter == $ad_threshold && $total_ads > 0) {
                            ...
                            if (!empty($current_ad)) {
                                if (strpos($current_ad, 'adsbygoogle') !== false) {
                                    echo '<div class="adt-adsense-wrapper">';
                                    echo $current_ad;
                                    echo '</div>';
                                } else {
                                    echo '<div class="adt-static-ad-wrapper">';
                                    echo $current_ad;
                                    echo '</div>';
                                }
                            }
    ...
    function adforest_ad_grid_1($ad_permalink, $first_img, $is_featured, $ad_categories_post, $ad_details, $truncated_title, $truncated_location, $price_html, $heart_class)
    {
        ...
        <div class="adt-category-ad-card">
            ...
            <div class="category-content-box">
                ...
                <div class="price-box">
                    <?php echo wp_kses($price_html, ADFOREST_ALLOWED_FORM_HTML); ?>
                    <a href="javascript:void(0);" class="favourite ad_to_fav" data-adid="<?php echo get_the_ID(); ?>"
    ...
    function adforest_ad_grid_2($all_ad_images, $ad_permalink, $is_featured, $ad_poster_img, $ad_poster_name, $ad_title, $truncated_location, $price_html, $heart_class, $post_id = null)
    {
        ...
        <div class="adt-property-ad-card">
            ...
            <div class="adt-property-price-box">
                ...
                <a href="javascript:void(0);" class="favorite ad_to_fav" data-adid="<?php echo get_the_ID(); ?>"
                   data-toggle="tooltip" data-placement="top"
                   title="<?php echo esc_attr__("Click to make it favourite", "adforest"); ?>">
                    <i class="<?php echo esc_attr($heart_class); ?>"></i>
                </a>
    ...
    function adforest_remove_expired_packages($user_id = "")
    {
        ...
        if (is_array($selected_categories) && count($selected_categories) > 0) {
            foreach ($selected_categories as $package_id => $details) {
                if (isset($details['pkg_expiry_days']) && $details['pkg_expiry_days'] != '-1' && strtotime($details['pkg_expiry_days']) < time()) {
...
if (!function_exists('adforest_get_message_count')) {
    function adforest_get_message_count()
    {
        ...
        $chat_unread = (int) $wpdb->get_var($wpdb->prepare("SELECT COUNT(*)
        ...
        if (function_exists('adforest_count_theme_notifications')) {
            $theme_unread = adforest_count_theme_notifications($current_user_id);
        }

        return $chat_unread + $theme_unread;
    }
}
...
/**
 * Nominatim/OpenStreetMap Proxy for Leaflet Maps
 * Prevents CORS issues by proxying requests through WordPress
 */
if (!function_exists('adforest_osm_nominatim_proxy')) {
    function adforest_osm_nominatim_proxy() {
        // Verify nonce for security
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'adforest_nominatim_proxy_nonce')) {
            wp_send_json_error(array('message' => __('Security check failed.', 'adforest')));
        }

        // Get and sanitize the query
        $query = isset($_POST['query']) ? sanitize_text_field(wp_unslash($_POST['query'])) : '';
        $limit = isset($_POST['limit']) ? absint($_POST['limit']) : 5;
        $addressdetails = isset($_POST['addressdetails']) ? absint($_POST['addressdetails']) : 1;

        if (empty($query)) {
            wp_send_json_error(array('message' => __('Query is required.', 'adforest')));
        }
        ...
        // Make the server-side request with proper User-Agent (required by Nominatim)
        $response = wp_remote_get($api_url, array(
            'timeout'    => 15,
            'user-agent' => 'AdForest-WordPress-Theme/1.0 (' . home_url() . ')',
            'headers'    => array(
        ...
        if (is_wp_error($response)) {
            wp_send_json_error(array(
                'message' => __('Failed to fetch location data.', 'adforest'),
        ...
        if ($response_code !== 200) {
            wp_send_json_error(array(
                'message' => __('Nominatim API returned an error.', 'adforest'),
                'code'    => $response_code,
            ));
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            wp_send_json_error(array('message' => __('Invalid response from Nominatim.', 'adforest')));
        }

        wp_send_json_success($data);
    }
}
add_action('wp_ajax_adforest_osm_nominatim_proxy', 'adforest_osm_nominatim_proxy');
add_action('wp_ajax_nopriv_adforest_osm_nominatim_proxy', 'adforest_osm_nominatim_proxy');
