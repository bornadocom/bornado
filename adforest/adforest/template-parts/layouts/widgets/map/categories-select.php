...
?>
<div class="adt-map-widget">
    <form method="get" action="<?php echo adforest_return_echo($adforest_search_page); ?>">
        <div class="form-group">
            <select id="adforest-category-select-map" class="form-control adforest-select2" data-placeholder="<?php echo esc_attr($placeholder); ?>" data-cascading="<?php echo $enable_cascading ? '1' : '0'; ?>" data-selected-chain="<?php echo esc_attr(implode(',', $selected_chain)); ?>">
        ...
        <div id="adforest-category-children-map" class="form-group" style="margin-top:10px;"></div>
        <?php } ?>
        <?php echo adforest_search_params('cat_id'); ?>
        <input type="hidden" name="cat_id" id="adforest-category-selected-map" value="<?php echo esc_attr($current_cat_id); ?>">
...
document.addEventListener('DOMContentLoaded', function(){
    if (window.jQuery && jQuery.fn.select2) {
        var $ = jQuery;
        var $root = $('#adforest-category-select-map');
        $root.select2({ width: '100%' });
        if ($root.data('cascading') == '1') {
            var $childrenWrap = $('#adforest-category-children-map');
            function fetchChildren(parentId, level, chainArr) {
                ...
                $.get('<?php echo admin_url('admin-ajax.php'); ?>', {
                    ...
                    if (resp && resp.success && resp.data && resp.data.length) {
                        var selectId = 'adforest-category-select-map-child-' + level;
                        ...
                        $sel.select2({ width: '100%' }).on('change', function(){
                            var val = $(this).val();
                            if (val) { $('#adforest-category-selected-map').val(val); fetchChildren(val, level + 1, chainArr); }
                        });
            ...
            $root.on('change', function(){
                var val = $(this).val();
                $('#adforest-category-selected-map').val(val);
                fetchChildren(val, 1, null);
            });
            var initial = $('#adforest-category-selected-map').val();
            ...
            if (chainArr && chainArr.length) {
                ...
                if ($root.find('option[value="' + rootTarget + '"]').length) {
                    ...
                    $('#adforest-category-selected-map').val(rootTarget);
