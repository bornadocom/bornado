...
if (isset($adforest_theme['search_popup_cat_disable']) && $adforest_theme['search_popup_cat_disable'] == true) {
    $ad_categories = array_filter($ad_categories, function($category) {
        $details = get_taxonomy_details($category);
        return isset($details['ad_count']) && $details['ad_count'] > 0;
    });
}
...
?>
<div class="adt-topbar-widget">
    <form method="get" action="<?php echo adforest_return_echo($adforest_search_page); ?>">
        <div class="form-group">
            <select id="adforest-category-select-topbar" class="form-control adforest-select2" data-placeholder="<?php echo esc_attr($placeholder); ?>" data-cascading="<?php echo $enable_cascading ? '1' : '0'; ?>" data-selected-chain="<?php echo esc_attr(implode(',', $selected_chain)); ?>">
                ...
                <?php foreach ($ad_categories as $category) {
                    ...
                    $label = $name;
                    if ($show_counts) { $label .= ' (' . intval($ad_count) . ')'; }
                    ?>
        ...
        <div id="adforest-category-children-topbar" class="form-group" style="margin-top:10px;"></div>
        <?php } ?>
        <?php echo adforest_search_params('cat_id'); ?>
        <input type="hidden" name="cat_id" id="adforest-category-selected-topbar" value="<?php echo esc_attr($current_cat_id); ?>">
...
document.addEventListener('DOMContentLoaded', function(){
    if (window.jQuery && jQuery.fn.select2) {
        var $ = jQuery;
        var $root = $('#adforest-category-select-topbar');
        $root.select2({ width: '100%' });
        if ($root.data('cascading') == '1') {
            var $childrenWrap = $('#adforest-category-children-topbar');
            function fetchChildren(parentId, level, chainArr) {
                ...
                loading.show();
                    ...
                    if (resp && resp.success && resp.data && resp.data.length) {
                        var selectId = 'adforest-category-select-topbar-child-' + level;
                        ...
                        $sel.select2({ width: '100%' }).on('change', function(){
                            var val = $(this).val();
                            if (val) { $('#adforest-category-selected-topbar').val(val); fetchChildren(val, level + 1, chainArr); }
                        });
            ...
            $root.on('change', function(){
                var val = $(this).val();
                $('#adforest-category-selected-topbar').val(val);
                fetchChildren(val, 1, null);
            });
            var initial = $('#adforest-category-selected-topbar').val();
            ...
            if (chainArr && chainArr.length) {
                ...
                if ($root.find('option[value="' + rootTarget + '"]').length) {
                    ...
                    $('#adforest-category-selected-topbar').val(rootTarget);
