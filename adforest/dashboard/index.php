                                ...
                                if ($total_unread > 0 && !empty($combined_notifications)) {
                                    ...
                                    foreach ($combined_notifications as $notification_item) {
                                        ...
                                        <?php
                                            ...
                                            $timestamp = isset($notification_item['timestamp']) ? (int) $notification_item['timestamp'] : time();
                                            // Re-translate the title/message at display time using the meta
                                            // event field. Notification rows store strings translated at create
                                            // time, so old rows can be in the wrong locale — this rebuilds them
                                            // against whatever locale the current request is using.
