<?php
if (!function_exists('adforest_get_menu_items_data')) {
    function adforest_get_menu_items_data($item_id, $item, $menu_level = 1, $show_indicator = true, $theme_location = "main_menu")
    {
        $item_data = array();
        $arr = array();
        $angleAerrow = '';
        global $wpdb;
        $metaID = is_numeric($item_id) ? intval($item_id) : 0;
        $count_children = $wpdb->get_var($wpdb->prepare("SELECT COUNT(meta_id) FROM $wpdb->postmeta WHERE meta_key='_menu_item_menu_item_parent' AND meta_value=%d", $metaID));
        ...
        if ($menu_level == 1) {
            ...
            $icon = ($menu_icon != "") ? ' <i class="' . esc_html($menu_icon) . ' custom-icon"></i>' : '';
            ...
            if ($menu_classes == "drop-down-multilevel") {
                ...
                $item_data['wrapHTMLStarts'] = '<ul class="' . esc_attr($cls) . '">';
                ...
                $item_data['wrapHTMLStarts'] = '<ul class="mega-menu-list ' . esc_attr($cls) . '"><li><div class="drop-down mega-menu-container" ><div class="grid-row row">';
        ...
        if ($menu_level == 3) {
            if ($show_indicator == true) {
                $indicatorL = ($menu_icon != "") ? ' <i class="' . esc_html($menu_icon) . ' custom-icon"></i>' : '<i class="fa ' . esc_attr($angle) . ' fa-indicator"></i>';
            } else {
                $indicatorL = ($menu_icon != "") ? ' <i class="' . esc_html($menu_icon) . ' custom-icon"></i>' : '';
            }
        }
...
if (!function_exists('adforest_theme_menu')) {
    function adforest_theme_menu($theme_location)
    {
        $menu_html = '';
            ...
            if (isset($menu->term_id)) {
                ...
                foreach ($menu_items as $item) {
                    ...
                    if ($item->menu_item_parent == 0) {
                        $level1 = 1;
                        $menuItems1 = adforest_get_menu_items_data($item->ID, $item, 1, true, $theme_location);
                        ...
                        foreach ($menu_items as $sub_item) {
                            if ($item->ID == $sub_item->menu_item_parent) {
                                ...
                                if ($mainMenuClass == 'drop-down' || $mainMenuClass == 'drop-down-multilevel') {
                                    $menuItems2 = adforest_get_menu_items_data($sub_item->ID, $sub_item, 2, true, $theme_location);
                                    if ($level1 == 1) {
                                    ...
                                    foreach ($menu_items as $sub_sub_item) {
                                        if ($sub_item->ID == $sub_sub_item->menu_item_parent) {
                                            ...
                                            if ($level2 == 1) {
                                                ...
                                                $isShowTitle = '<h4 ><a href="' . esc_url($menuItems2['megamenu_menu_url']) . '">' . esc_html($menuItems2['megamenu_menu_title']) . '</a></h4>';
                                            ...
                                            $menuItems3 = adforest_get_menu_items_data($sub_sub_item->ID, $sub_sub_item, 3, $show_indicator, $theme_location);
        ...
        echo adforest_return_echo($menu_html);
    }
}
...
if (!class_exists('adforest_Walker_Nav_Menu_Edit')) {
    class adforest_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit
    {
        public function start_el(&$output, $data_object, $depth = 0, $args = null, $current_object_id = 0)
        {
            ...
            if ($new_fields = adforest_Nav_Menu_Item_Custom_Fields::get_field($data_object, $depth, $args)) {
