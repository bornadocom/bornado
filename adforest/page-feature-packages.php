...
if ($category_pkg_posts->have_posts()) {
    while ($category_pkg_posts->have_posts()) {
        ...
        $Featured_expiry_days = get_post_meta($post_id, 'ppp_package_adFeatured_expiry_days', true);
            ...
            <div class="row">
                <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12">
                    <div class="uploaded-ad-box">
                        <div class="left-icon">
                            <img src="<?php echo esc_url(get_template_directory_uri()) . "/images/hand-shake.svg" ?>"
                                 alt="<?php echo esc_attr__( 'hand-shake', 'adforest' ); ?>">
                        </div>
                    ...
                    <form id="feature_ad_form" class="feature_ad_form">
                            ...
                            <div class="row">
                                <div class="col-12 col-sm-12 col-md-12 col-lg-8 col-xl-8 col-xxl-8">
                                    <div class="upgrade-position-detail">
                                        ...
                                        <ul>
                                            <li>
                                                <img src="<?php echo esc_url(get_template_directory_uri()) . "/images/tick-img.png" ?>"
                                                     alt="<?php echo esc_attr__( 'tick-icon', 'adforest' ); ?>">
                                                <?php echo esc_html__('Get noticed with', 'adforest'); ?>
                                                <span><?php echo esc_html__('"featured"', 'adforest'); ?></span><?php echo esc_html__('tag in top position', 'adforest'); ?>
                                            </li>
                                            <li>
                                                <img src="<?php echo esc_url(get_template_directory_uri()) . "/images/tick-img.png" ?>"
                                                     alt="<?php echo esc_attr__( 'tick-icon', 'adforest' ); ?>">
                                ...
                                <div class="col-12 col-sm-12 col-md-12 col-lg-4 col-xl-4 col-xxl-4">
                                    <div class="upgrade-image-box">
                                        <!-- <img src="<?php echo esc_url(get_template_directory_uri()) . "/images/featured-ad.png" ?>"
                                             alt="featured-ad"> -->
                                    </div>
                                    <?php
                                        $location_limit = 40;
                                        $title_limit = 40;
                                        $ad_details = $pid > 0 && function_exists('get_ad_post_details')
                                            ? get_ad_post_details($pid)
                                            : array();
                                        if (!empty($ad_details)) {
                                            $first_img          = isset($ad_details['img']) ? $ad_details['img'] : '';
                                            $truncated_location = truncate_string(isset($ad_details['location']) ? $ad_details['location'] : '', $location_limit);
                                            $truncated_title    = truncate_string(isset($ad_details['ad_title']) ? $ad_details['ad_title'] : '', $title_limit);
                                            $price_html         = isset($ad_details['price_html']) ? $ad_details['price_html'] : '';
                                            $ad_permalink       = isset($ad_details['ad_link']) ? $ad_details['ad_link'] : '';
                                            $heart_class        = isset($ad_details['heart_class']) ? $ad_details['heart_class'] : '';
                                            $is_featured        = isset($ad_details['is_featured']) ? $ad_details['is_featured'] : '';
                                            $all_ad_images      = isset($ad_details['all_ad_images']) ? $ad_details['all_ad_images'] : array();
                                            ...
                                            $ad_title           = truncate_string(isset($ad_details['ad_title']) ? $ad_details['ad_title'] : '', $title_limit);
                                            $ad_categories_post = isset($ad_details['categories']) && is_array($ad_details['categories']) ? $ad_details['categories'] : array();
                                            $featured_tag       = $is_featured ? '<img style="transform: rotate(180deg);" src="' . esc_url(get_template_directory_uri()) . '/images/featured.png' . '" alt="' . esc_attr__( 'featured-tag', 'adforest' ) . '" class="featured-tag">' : '';
                                    ...
                                    <ul class="featured-duration-list">
                                        ...
                                        if ($category_pkg_posts->have_posts()) {
                                            while ($category_pkg_posts->have_posts()) {
                                                ...
                                                $Featured_expiry_days = get_post_meta($post_id, 'ppp_package_adFeatured_expiry_days', true);
                                                $ads_for_days = $post_title . ' ' . __('For', 'adforest') . ' ' . $Featured_expiry_days . ' ' . __(' Days', 'adforest');
                                                $regular_price = get_post_meta($post_id, '_regular_price', true);
                                                $sale_price = get_post_meta($post_id, '_sale_price', true);
                                                ?>
                                                <li>
                        ...
                        <input type="hidden" name="pid" id="pid" value="<?php echo esc_attr($pid); ?>">
                        <input type="hidden" id="sb-post-feature-token"
                               value="<?php echo wp_create_nonce('sb_post_feature_secure'); ?>"/>
                        <div class="ad-botm-buttons">
                            <a href="<?php echo get_the_permalink($pid); ?>"
                               class="skip-btn adt-button-dark-1"><?php echo esc_html__('Skip, View Your Ad ', 'adforest'); ?></a>
                            <button type="submit"
                                    class="upgrade-btn adt-button-dark"><?php echo esc_html__('Upgrade Your Ad ', 'adforest'); ?></button>
                        </div>
                    </form>
                </div>
            </div>
