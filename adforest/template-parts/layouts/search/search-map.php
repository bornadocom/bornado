...
<section class="adt-map-search-section">
    <div class="map-search-wrapper">
        <div class="search-content-side scroller">
                ...
                <div class="search-filters-wrapper">
                    <div class="dropdown adtype-dropdown">
                        ...
                        <form id="ad_type_form" method="get"
                            ...
                            $type_placeholder = $searched_type_name == "" ? esc_html__( 'Ad Type', 'adforest' ) : $searched_type_name;
                            ?>
                    ...
                    <div class="dropdown category-dropdown adt-category-list-sidebar">
                        ...
                        <form method="get" id="search_cats_w"
                            ...
                            $cat_placeholder = $searched_cat_name == "" ? esc_html__( 'All Categories', 'adforest' ) : $searched_cat_name;
                            ?>
                            <button class="btn btn-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown"
                                    aria-expanded="false">
            ...
            // The map layout splits the viewport ~40% ads / ~60% map. Four
            // cards per row would either overflow into the map area or
            // shrink past readability, so cap to 3 (the 4th wraps to the
            // next row). Also pair the track with `minmax(0, 1fr)` instead
            // of plain `1fr` — the default `minmax(auto, 1fr)` lets card
            // content push tracks wider than their share and clip behind
            // the map column.
            if ( (int) $grid_cols > 3 ) {
                $grid_cols = 3;
            }
            ...
            if (($query->have_posts() && isset($_GET['view-type']) && $_GET['view-type'] != 'list') || ($query->have_posts() && !isset($_GET['view-type']))) { ?>
                <div class="search-ads-result-box grid <?php echo esc_attr($sb_2column) ?>"
                     style="grid-template-columns: repeat(<?php echo esc_attr($grid_cols); ?>, minmax(0, 1fr));">
                    <?php
            ...
            <?php } else {
                ...
                echo '<div class="no_ads_found">
                    ...
                    <h3>' . esc_html__( 'No Ads found.', 'adforest' ) . '</h3>
                  </div>';
            } ?>
            <?php
            if ($query->have_posts()) {
        ...
        <div class="search-map-side" style="height: 100vh;">
            ...
            $get_directions_text = esc_html__( 'Get Directions', 'adforest' );
            $ads = [];
            if ($query->have_posts()) {
                while ($query->have_posts()) {
                    $query->the_post();
