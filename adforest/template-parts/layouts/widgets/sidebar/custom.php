                ...
                if ($r['titles'] != "" && $r['slugs'] != "" && $in_search == 1) {
                    ...
                    $customHTML .= '<div class="accordion-item">
                                      <div class="accordion-header" role="tab" id="heading-' . $rand_ids . '">
                                         <button class="accordion-button '. $collapsed .'" type="button" data-bs-toggle="collapse" data-bs-target="#panelsStayOpen-collapse'.$custom_id.'" aria-expanded="false" aria-controls="panelsStayOpen-collapse'.$custom_id.'-collapseFive" fdprocessedid="1bxzi">' . esc_html($title = empty($instance['title']) ? '' : apply_filters('widget_title', $instance['title'])) . ' ' . esc_html($r['titles']) . '</button>
                                         </div>
				...
				<div class="col-md-12 no-padding"><button type="submit" class="' . esc_attr($btn_cls) . '"><i class="fa fa-search"></i></button></div>';
                    ...
                    if (isset($r['types']) && $r['types'] == 6) {
                        if ( function_exists( 'wp_enqueue_script' ) ) {
                            if ( ! wp_script_is( 'rangeslider-min', 'enqueued' ) ) {
                                wp_enqueue_script( 'rangeslider-min' );
                            }
                        }

                        if ( function_exists( 'wp_enqueue_style' ) ) {
                            if ( ! wp_style_is( 'rangeslider-css', 'enqueued' ) ) {
