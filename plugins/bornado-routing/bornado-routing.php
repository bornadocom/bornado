...
final class Bornado_SEO_Routing {
	...
	/**
	 * Convert taxonomy popup/widget URLs to contextual semantic routes.
	 *
	 * @param string $url Existing URL.
	 * @param int    $tax_id Term ID.
	 * @param string $tax_name Taxonomy name.
	 * @return string
	 */
	public static function filter_taxonomy_popup_action( $url, $tax_id = '', $tax_name = 'ad_cats' ) {
		$term_id = max( 0, (int) $tax_id );
		if ( $term_id < 1 ) {
			return $url;
		}

		if ( 'ad_cats' === $tax_name ) {
			return self::get_contextual_url(
				array(
					'cat_id'       => $term_id,
					'fallback_url' => $url,
				)
		...
		if ( 'ad_country' !== $tax_name ) {
			return $url;
		}

		$route_terms = self::get_route_terms_from_location_id( $term_id );
		if ( ! ( $route_terms['country_term'] instanceof WP_Term ) ) {
			return $url;
		}

		$current_cat = self::get_selected_category_id_from_args( self::get_request_query_args() );
		if ( ! $current_cat && self::is_current_seo_route() && ! empty( self::$context['deepest_term'] ) && self::$context['deepest_term'] instanceof WP_Term ) {
			$current_cat = (int) self::$context['deepest_term']->term_id;
		} elseif ( ! $current_cat && is_tax( 'ad_cats' ) ) {
			$queried_term = get_queried_object();
			if ( $queried_term instanceof WP_Term ) {
				$current_cat = (int) $queried_term->term_id;
			}
		}

		$semantic_url = self::build_semantic_url(
			(int) $route_terms['country_term']->term_id,
			$route_terms['city_term'] instanceof WP_Term ? (int) $route_terms['city_term']->term_id : 0,
			$current_cat
		);
	...
	/**
	 * Keep robots directives aligned with semantic-route indexation policy.
	 *
	...
	public static function get_public_route_context() {
		...
		if ( empty( self::$context ) ) {
			$context['public_query'] = self::get_public_query_args();
			return $context;
		}
		...
		if ( ! empty( self::$context['country_term'] ) && self::$context['country_term'] instanceof WP_Term ) {
			...
			$context['country_id']   = (int) self::$context['country_term']->term_id;
		}

		if ( ! empty( self::$context['city_term'] ) && self::$context['city_term'] instanceof WP_Term ) {
			$context['city_term'] = self::$context['city_term'];
			$context['city_id']   = (int) self::$context['city_term']->term_id;
		}
		...
		if ( ! empty( self::$context['deepest_term'] ) && self::$context['deepest_term'] instanceof WP_Term ) {
			...
			$context['cat_id']       = (int) self::$context['deepest_term']->term_id;
		}
	...
	/**
	 * Return the current request query args after removing structural route params.
	 *
	 * @param array<int,string> $excluded_keys Keys to exclude from the result.
	 * @return array<string,mixed>
	 */
	public static function get_public_query_args( $excluded_keys = array() ) {
		$args = self::sanitize_public_query_args( self::get_public_query_state( self::get_request_query_args() ) );
	...
	/**
	 * Build a search/archive URL anchored to the current semantic context.
	 *
	 * @param array<string,mixed> $args Optional overrides.
	 * @return string
	 */
	public static function get_contextual_url( $args = array() ) {
		$args = wp_parse_args(
			$args,
			array(
				'country_id'            => null,
		...
		if ( self::is_current_seo_route() && ! empty( self::$context['deepest_term'] ) && self::$context['deepest_term'] instanceof WP_Term ) {
			$current_cat = self::$context['deepest_term'];
		} elseif ( is_tax( 'ad_cats' ) ) {
			$queried_term = get_queried_object();
			if ( $queried_term instanceof WP_Term ) {
				$current_cat = $queried_term;
			}
		}

		$country_id = null !== $args['country_id'] ? max( 0, (int) $args['country_id'] ) : ( $active_country instanceof WP_Term ? (int) $active_country->term_id : 0 );
		$city_id    = null !== $args['city_id'] ? max( 0, (int) $args['city_id'] ) : ( $active_city instanceof WP_Term ? (int) $active_city->term_id : 0 );
		$cat_id     = null !== $args['cat_id'] ? max( 0, (int) $args['cat_id'] ) : ( $current_cat instanceof WP_Term ? (int) $current_cat->term_id : 0 );

		$query_args = array();
		if ( ! empty( $args['preserve_public_query'] ) ) {
			$query_args = self::get_public_query_args();
		}
		if ( is_array( $args['query_args'] ) ) {
			$query_args = array_merge( $query_args, $args['query_args'] );
		}
		$query_args = self::sanitize_public_query_args( $query_args );

		$paged = null !== $args['paged'] ? max( 1, (int) $args['paged'] ) : 1;
		if ( null === $args['paged'] ) {
			$paged = ! empty( self::$context['paged'] ) ? max( 1, (int) self::$context['paged'] ) : 1;
		}
		unset( $query_args['paged'], $query_args['page'] );

		$semantic_url = self::build_semantic_url( $country_id, $city_id, $cat_id, $paged, $query_args );
		if ( '' !== $semantic_url ) {
			return $semantic_url;
		}
		...
		if ( '' === $fallback_url ) {
			$fallback_url = self::get_search_page_base_url();
		}
		if ( '' === $fallback_url ) {
			return '';
		}

		$location_id = $city_id > 0 ? $city_id : $country_id;
		if ( $location_id > 0 ) {
			$query_args['country_id'] = $location_id;
		}
		if ( $cat_id > 0 ) {
		...
		return empty( $query_args ) ? $fallback_url : add_query_arg( $query_args, $fallback_url );
	}
	...
	/**
	 * Add runtime-only data such as canonical URL and matching landing post.
	 *
	...
	private static function hydrate_route_context( array $resolved ) {
		...
		$resolved['paged'] = max( 1, (int) $resolved['paged'] );
		if ( empty( $resolved['is_valid'] ) ) {
			$resolved['landing_post'] = null;
			return $resolved;
		}

		if ( '' === (string) $resolved['canonical_url'] ) {
			$resolved['canonical_url'] = self::build_semantic_url(
				$resolved['country_term'] instanceof WP_Term ? (int) $resolved['country_term']->term_id : 0,
				$resolved['city_term'] instanceof WP_Term ? (int) $resolved['city_term']->term_id : 0,
				$resolved['deepest_term'] instanceof WP_Term ? (int) $resolved['deepest_term']->term_id : 0,
				(int) $resolved['paged']
			);
		}
	...
	/**
	 * Bind a resolved route to the correct renderable WP object.
	 *
	...
	private static function get_bound_post( array $route_context ) {
		...
		return $search_page instanceof WP_Post ? $search_page : null;
	}

	/**
	 * Convert a bound post into request vars consumed by WordPress.
	 *
	 * @param array<string,mixed> $route_context Hydrated route context.
	 * @return array<string,mixed>
	 */
	private static function get_bound_query_vars( array $route_context ) {
		...
		if ( ! empty( $route_context['segments'] ) && is_array( $route_context['segments'] ) ) {
			$query_vars[ self::QUERY_ROUTE_PATH ] = implode( '/', array_map( 'sanitize_title', $route_context['segments'] ) );
		}
		...
		if ( ! empty( $route_context['landing_post'] ) && $route_context['landing_post'] instanceof WP_Post ) {
			$query_vars['post_type'] = $bound_post->post_type;
			$query_vars['p']         = $bound_post->ID;
		} else {
	...
	/**
	 * Resolve a city term that belongs to the supplied country branch.
	 *
	 * @param string  $slug City slug.
	 * @param WP_Term $country_term Root country term.
	 * @return WP_Term|null
	 */
	...
	/**
	 * Split an AdForest location term into country and city route terms.
	 *
	...
	private static function split_location_term_for_route( $term ) {
		...
		if ( empty( $ancestors ) ) {
			$route_terms['country_term'] = $term;
			return $route_terms;
		}

		$country_term = get_term( (int) $ancestors[0], 'ad_country' );
		if ( $country_term instanceof WP_Term ) {
			$route_terms['country_term'] = $country_term;
			$route_terms['city_term']    = $term;
		}
	...
	/**
	 * Resolve route terms from a raw location identifier.
	 *
	 * @param int $location_id Country or city term ID.
	 * @return array{country_term:WP_Term|null,city_term:WP_Term|null}
	 */
	private static function get_route_terms_from_location_id( $location_id ) {
		$location_id = max( 0, (int) $location_id );
		if ( $location_id < 1 ) {
			return array(
				'country_term' => null,
				'city_term'    => null,
		...
		return self::split_location_term_for_route( $term );
	}

	/**
	 * Return whether a location term belongs to a given country term.
	 *
	 * @param WP_Term|int $term Location term or ID.
	 * @param int         $country_term_id Country term ID.
	 * @return bool
	 */
	private static function is_term_within_country( $term, $country_term_id ) {
	...
	/**
	 * Return the permalink that should be used for legacy fallback URLs.
	 *
	 * @return string
	 */
	private static function get_search_page_base_url() {
		$search_page_id = self::get_adforest_search_page_id();
		if ( $search_page_id < 1 ) {
			return home_url( '/' );
		}
	...
	/**
	 * Remove structural routing params and empty values from public query args.
	 *
	 * @param array<string,mixed> $query_args Raw query args.
	 * @return array<string,mixed>
	 */
	private static function sanitize_public_query_args( array $query_args ) {
		unset(
			$query_args['country_id'],
			...
			$query_args['ad_cat_sub_sub_sub'],
			$query_args['ad_cat_sub_sub_sub_sub'],
			$query_args['bornado_debug_route']
		);
		...
		foreach ( $query_args as $key => $value ) {
			...
			$normalized = self::normalize_public_query_value( $value );
			if ( null === $normalized || ( is_array( $normalized ) && empty( $normalized ) ) ) {
				unset( $query_args[ $key ] );
				continue;
			}
	...
	/**
	 * Normalize a public query value and drop empty payloads.
	 *
	 * @param mixed $value Raw query value.
	 * @return mixed|null
	 */
	private static function normalize_public_query_value( $value ) {
		if ( is_array( $value ) ) {
			$normalized = array();
			foreach ( $value as $child_key => $child_value ) {
				$child = self::normalize_public_query_value( $child_value );
				if ( null !== $child && ! ( is_array( $child ) && empty( $child ) ) ) {
	...
	/**
	 * Resolve the deepest selected category from an AdForest-style argument array.
	 *
	...
	private static function get_selected_category_id_from_args( array $args ) {
		...
		foreach ( $keys as $key ) {
			...
			$value = is_array( $args[ $key ] ) ? reset( $args[ $key ] ) : $args[ $key ];
...
if ( ! function_exists( 'bornado_seo_routing_get_public_query_args' ) ) {
	/**
	 * Public helper returning non-structural query args for the current request.
	 *
	 * @param array<int,string> $excluded_keys Keys to exclude.
	 * @return array<string,mixed>
	 */
	function bornado_seo_routing_get_public_query_args( $excluded_keys = array() ) {
		if ( ! class_exists( 'Bornado_SEO_Routing' ) ) {
			return array();
		}

		return Bornado_SEO_Routing::get_public_query_args( $excluded_keys );
	}
}

if ( ! function_exists( 'bornado_seo_routing_get_contextual_url' ) ) {
	/**
	 * Public helper building a contextual semantic URL.
	 *
	 * @param array<string,mixed> $args Optional overrides.
	 * @return string
	 */
	function bornado_seo_routing_get_contextual_url( $args = array() ) {
		if ( ! class_exists( 'Bornado_SEO_Routing' ) ) {
			return '';
		}

		return Bornado_SEO_Routing::get_contextual_url( $args );
	}
}
