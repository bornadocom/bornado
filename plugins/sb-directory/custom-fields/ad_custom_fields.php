...
class Sb_Acf_Fields
{
    ...
    {
        add_action('l_event_cat_add_form_fields', array($this, 'exertio_framework_acf_field_term_meta_cat_template'), 10, 1);
        add_action('l_event_cat_edit_form_fields', array($this, 'exertio_framework_acf_field_term_meta_cat_template'), 10, 1);
        add_action('edit_l_event_cat', array($this, 'exertio_framework_acf_save_term_meta_cat_template'));
        add_action('create_l_event_cat', array($this, 'exertio_framework_acf_save_term_meta_cat_template'));
        ...
        add_filter('dwt_listing_framework_acf_frontend_html', array($this, 'exertio_framework_acf_show_frontend_fields_callback'), 10, 5);
        add_filter('sb_framework_acf_frontend_html_search', array($this, 'exertio_framework_acf_show_frontend_fields_search_callback'), 10, 5);
    }
                ...
                <div class="<?php echo esc_attr($col_class); ?>">
                    <div class="<?php echo esc_attr($form_class); ?>">
                        ...
                        if ($cus_type == 'search-page') {
                            ...
                            if ($field_val['type'] == 'true_false') {
                                ...
                                <div class="pretty p-icon p-toggle p-plain mw-true-false ">
                                    ...
                                    <input type="checkbox" name="acf[<?php echo esc_attr($field_val['key']); ?>]" autocomplete="off"
                                        value="1" />
                        ...
                        <input type="text" class="custom-range-slider" name="acf[<?php echo esc_attr($field_val['key']); ?>]" value=""
                            data-type="double" data-min="<?php echo esc_attr($field_val['min']); ?>"
                            data-max="<?php echo esc_attr($field_val['max']); ?>" data-step="<?php echo esc_attr($field_val['step']); ?>"
                            data-from="<?php echo esc_attr($m_from); ?>" data-to="<?php echo esc_attr($m_to); ?>" data-grid="true" />
                        </div>
                        </div>
                        <?php
                        continue;
    ...
    function exertio_framework_acf_show_frontend_fields_search_callback($html_data = '', $fields_data = array(), $post_id = 0, $col = 'col-6', $cus_type = 'default')
    {
        $html_data = '';
        if (isset($fields_data) && is_array($fields_data) && sizeof($fields_data) > 0) {
            ob_start();
    ...
    function exertio_framework_acf_field_term_meta_cat_template($taxonomy)
    {
        ...
        <tr class="form-field term-parent-wrap">
            <th scope="row"><label for="parent"><?php _e('Select Template', 'sb_framework'); ?></label></th>
            <td>
                <select name="exertio_acf_template">
                    <option value=""><?php echo __('Select An Option', 'sb_framework'); ?></option>
                    <option value="no_temp"><?php echo __('No Template', 'sb_framework'); ?></option>
                    <?php echo ($template_html); ?>
                </select>
                <p class="description"><?php echo __('You can assign this template each level category.', 'sb_framework'); ?>
                </p>
                <br />
            </td>
        </tr>
...
class exertio_framework_cat_template extends ACF_Location
{
    public function initialize()
    {
        ...
        $this->label = __("Adforest Category Template", 'sb_framework');
    }

    public function get_values($rule)
    {
        $choices = array();
        $choices['cat-template'] = __("Adforest Category Template", 'sb_framework');
        return $choices;
    }
}
class exertio_framework_cat_template_wpml extends ACF_Location
{
    public function initialize()
    {
        ...
        $this->label = __("Adforest Category Template WPML", 'sb_framework');
    }

    public function get_values($rule)
    {
        $choices = array();
        $choices['cat-template-wpml'] = __("Adforest Category Template WPML", 'sb_framework');
        return $choices;
    }
}
...
function exertio_framework_acf_render_field_settings($field)
{
    acf_render_field_setting($field, array(
        'label' => __('Display in search', 'sb_framework'),
        'instructions' => __('Enable/Disable this field to show in search page.', 'sb_framework'),
        'name' => 'in_search',
        'type' => 'true_false',
        'ui' => 1,
    ), true);
...
if (!function_exists('sb_framework_fields_by_listing_id')) {

    function sb_framework_fields_by_listing_id($classified_id = '')
    {
        $cats_value = get_post_meta($classified_id, 'adforest_event_category', true);
        // return $cats_value;
...
//custom fields
add_action('wp_ajax_sb_get_custom_fields', 'sb_framework_get_custom');
add_action('wp_ajax_nopriv_sb_get_custom_fields', 'sb_framework_get_custom');
if (!function_exists('sb_framework_get_custom')) {
    function sb_framework_get_custom()
    {
        error_log("In This");
        if(!class_exists("ACF")) {
            error_log("In that 1");
        ...
        if (!empty($_POST['cat_parent'])) {
            $col = '';
            $cus_type = $category_id = $result = '';
            $category_details = array();
            $parent_category = $_POST['cat_parent'];
            $category_details = get_term_by('id', $parent_category, 'l_event_cat');
            if (!empty($category_details)) {
                ...
                if (isset($template_group_key) && $template_group_key != '' && class_exists('ACF')) {
                    $fields_data = acf_get_fields($template_group_key);
                    $custom_fields_html = apply_filters('dwt_listing_framework_acf_frontend_html', '', $fields_data, 0, $col, $cus_type);
...
//custom fields
add_action('wp_ajax_sb_get_custom_fields_search', 'sb_framework_get_custom_search');
add_action('wp_ajax_nopriv_sb_get_custom_fields_search', 'sb_framework_get_custom_search');
if (!function_exists('sb_framework_get_custom_search')) {
    function sb_framework_get_custom_search()
    {
        if (!empty($_POST['cat_parent'])) {
            $col = '';
            $cus_type = $category_id = $result = '';
            ...
            if (!empty($category_details)) {
                ...
                if (isset($template_group_key) && $template_group_key != '' && class_exists('ACF')) {
                    $fields_data = acf_get_fields($template_group_key);
                    $custom_fields_html = apply_filters('sb_framework_acf_frontend_html_search', '', $fields_data, 0, $col, $cus_type);
                    if (!empty($custom_fields_html)) {
                        $return = array('fields' => $custom_fields_html);
                        return wp_send_json_success($return);
                    } else {
