    ...
    if (isset($cus_fields) && !empty($cus_fields) && is_array($cus_fields) && count($cus_fields) > 0) {
        ...
        <div>
            <!-- <div class="panel-heading">
                <h3 class="panel-title"><a href="javascript:void(0)"> <i
                            class="ti-server"></i><?php echo esc_html($main_title); ?></a></h3>
            </div> -->
            <?php
            if (isset($custom_data) && !empty($custom_data) && is_array($custom_data) && count($custom_data) > 0) {
                ...
                <div>
                    ...
                    foreach ($custom_data as $data) {
                        ...
                        ?>
                        <div>
                            <div class="sb-specs-row">
                                <div class="event-acf-fields">
                                    <p><?php echo esc_html($data['label']) . ": "; ?></p>
                                    <h6>
                                        <?php
                                        if ($data['type'] == 'url') {
                                            echo '<a href="' . esc_url($data['value']) . '" target="_blank">' . esc_html__('Link', 'sb-directory') . '</a>';
                                        } else if ($data['type'] == 'true_false') {
                                            if ($data['value'] == 1 && $data['ui'] == 1) {
                                                echo esc_html($data['ui_on_text']);
                                            } else {
                                                echo esc_html__('Yes', 'sb-directory');
                                            }
                                        } else {
            ...
            foreach ($custom_data as $data) {
                if ($data['type'] == 'textarea' && !empty($data['value'])) {
                    ?>
                    <div class="widget-seprator sb-acf-custom-textarea clearfix">
                ...
                if ($data['type'] == 'select' && isset($data['is_multiple']) && $data['is_multiple'] == 1) {
                    ?>
                    <div class="widget-seprator sb-acf-custom-multiple listing-features clearfix">
                        <div class="widget-seprator-heading">
                            <h3 class="sec-titles"><?php echo esc_html($data['label']) . ": "; ?></h3>
                        </div>
                ...
                if ($data['type'] == 'checkbox' && is_array($data['value']) && count($data['value']) > 0) {
                    ?>
                    <div class="widget-seprator dwt-acf-custom-checkbox listing-features clearfix">
                        <div class="widget-seprator-heading">
                            <h3 class="sec-titles"><?php echo esc_html($data['label']) . ": "; ?></h3>
                        </div>
                        <ul class="list-unstyled">
