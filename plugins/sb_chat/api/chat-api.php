...
function sbchat_rest_send_message(WP_REST_Request $request)
{
    ...
    /* AdForest patch (per-ad conversations):
       When post_id is supplied, ALWAYS resolve the conversation via
       (recipient, post_id) so ad-scoped chats stay separate even when the
       client passes a stale/legacy conversation_id from an unrelated ad.
       Mirrors sb_send_message_ajax_callback() in utilities.php so REST API
       consumers (mobile apps, external integrations) match the web UI's
       behavior exactly.
       - post_id > 0  : find-or-create by (recipient, post_id); start_new and
                        client-supplied conversation_id are intentionally
                        ignored — the lookup is authoritative.
       - start_new    : create a fresh legacy (NULL post_id) conversation,
                        preserving the historical REST contract.
       - default      : trust the supplied conversation_id, validate access. */
    if ($post_id > 0) {
        $existing = $chat->sb_conversation_exists($recipient_id, $post_id);
    ...
    if ($conversation_id <= 0) {
        return new WP_REST_Response([
            'success' => false,
            'message' => __('Conversation could not be resolved or created.', 'sb_chat')
        ], 500);
    }
    ...
    if ($post_id > 0) {
        ...
        if ($post) {
            /* AdForest patch (post-card de-duplication):
               Only prepend the ad post-card on the FIRST message of the
               conversation. Repeated Contact-Seller submits on the same
               ad were prepending a card to every message, so the thread
               showed the same ad card stacked over and over. The modern
               chat header already shows "From <ad>" persistently, so
               the inline card just needs to anchor the top of the
               thread once. At this point $conversation_id is fully
               resolved (find-or-created above) — 0 messages = fresh
               row, > 0 = follow-up, skip the prepend. */
            global $wpdb;
            $msg_tbl_for_card = $wpdb->prefix . 'sb_chat_messages';
            $existing_msg_count = (int) $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM $msg_tbl_for_card WHERE conversation_id = %d",
                (int) $conversation_id
            ));
...
//            error_log('SB Chat: Message contains 4-byte UTF-8 characters (emojis): ' . $message);
        ...
        if ($message_sent) {
            if (function_exists('adforestAPI_messages_sent_func')) {
                ...
                error_log("Conversation ID before sending notification: " . $conversation_id);
...
add_action('rest_api_init', function () {
    register_rest_route('sbchat/v1', '/check-conversation', [
        ...
        'args' => [
            ...
            /* AdForest patch (per-ad conversations): callers that originate
               from an ad detail page should pass post_id so the lookup
               returns the ad-scoped conversation, not an unrelated legacy
               thread between the same pair. Omit (or pass 0) for the
               legacy / generic lookup. */
            'post_id' => [
                'required' => false,
                'type' => 'integer',
                'default' => 0,
...
function sbchat_api_check_conversation(WP_REST_Request $request)
{
    ...
    /* AdForest patch (per-ad conversations):
       - post_id > 0  : return the conversation scoped to that exact ad.
       - post_id == 0 : return the legacy / generic conversation (post_id
                        column NULL or 0). Never collapses ad-scoped rows
                        into the legacy match, which would have caused the
                        client to overwrite an unrelated thread.
       The per-ad clause is wrapped in a column existence check so the
       endpoint stays functional on installs where the schema migration in
       sb-chat-modern-extensions.php hasn't run yet (e.g. plugin-only
       install, theme not active). */
    $has_post_col = (bool) $wpdb->get_var($wpdb->prepare(
        "SHOW COLUMNS FROM $table LIKE %s",
        'post_id'
    ));

    if ($post_id > 0 && $has_post_col) {
        $conversation = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT id, user_1, user_2, deleted_by_user_1, deleted_by_user_2, created, updated
                 FROM $table
                 WHERE post_id = %d
                   AND ((user_1 = %d AND user_2 = %d)
    ...
    $response_data = [
        ...
        'recipient_id' => $recipient_id,
        // Echo back the resolved post_id scope so the client knows which
        // ad context this lookup answered for (helps debugging multi-ad
        // chats and lets the caller cache by scope).
        'post_id' => $post_id,
    ];
