    ...
    jQuery(document).ready(function () {
        ...
        if (document.getElementById('sbchat-mu')) {
            ...
            var inboxDropzone = new Dropzone('#sbchat-mu', {
                ...
                dictCancelUpload: (localize_vars.i18n && localize_vars.i18n.cancelUpload) || 'Cancel Upload',
            ...
            inboxDropzone.on('error', function(file, errorMessage) {
                ...
                if (file.previewElement) {
                    var uploadFailedTpl = (localize_vars.i18n && localize_vars.i18n.uploadFailed) || 'Upload failed: %s';
                    $(file.previewElement).append('<div class="dz-error-message">' + uploadFailedTpl.replace('%s', errorMessage) + '</div>');
                }
            });

            inboxDropzone.on('maxfilesexceeded', function () {
                inboxDropzone.removeAllFiles(true);
                var maxFilesTpl = (localize_vars.i18n && localize_vars.i18n.maxFilesExceeded) || 'Only %s files can be uploaded at a time.';
                var error = '<p class="maxfilesexceeded error" style="display: none; color: red;">' + maxFilesTpl.replace('%s', inboxDropzone.options.maxFiles) + '</p>';
        ...
        $(document).on('submit', '.send-message', function (e) {
            ...
            $sendBtn.html('<i class="fa fa-spinner fa-spin"></i> ' + ((localize_vars.i18n && localize_vars.i18n.sending) || 'Sending...'));
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.success) {
                        ...
                        var loadMoreTpl = (localize_vars.i18n && localize_vars.i18n.loadMore) || 'Load more (%s)';
                        load_conversations_btn.text(loadMoreTpl.replace('%s', new_offset));
