jQuery(document).ready( function( $ ) {
   ...
   $(document).on('click', '.clear_sb_data', function (e) {
       e.preventDefault();
       var clearMsg = (typeof sbchatLabels !== 'undefined' && sbchatLabels.clearDataConfirm) ? sbchatLabels.clearDataConfirm : 'Are you sure you want to clear plugin data?';
       const confirmed = confirm(clearMsg);
...
function initialize_search_select2() {
    ...
    var defaultChoice = { id: -1, text: (typeof sbchatLabels !== 'undefined' && sbchatLabels.selectUser) ? sbchatLabels.selectUser : '— Select User —' };
    ...
    $(document).on('click', '.block-user-admin', function () {
        ...
        if (confirm(block_text)) {
            $.post(ajaxurl, {action: 'sb_block_single_user', user_id: user_id , from_admin : 'yes' ,  'security': sbchat.ajax_nonce, 'block_status' : block_status}).done(function (response) {
               ...
               alert((typeof sbchatLabels !== 'undefined' && sbchatLabels.securityIssue) ? sbchatLabels.securityIssue : 'Security issue, please verify your nonce');
            });
        }
    });
