    ...
    foreach ( $inbox_conversations as $inbox_conversation ) {
        ...
        if ( ! empty( $message ) ) {
            ...
            MESSAGE_HTML;
                    ...
                    // The `|` inside the marker is the duration delimiter, NOT the
                    // legacy ad-card/text separator. Exploding on it would tear the
                    // marker in half and the client-side decorator (regex requires
                    // the full marker in one text node) would never run.
                    ...
                    if ( trim( $message_esc ) !== '' ) {
                        ...
                        // `[adf-voice]URL|DURATION[/adf-voice]` text until the JS
                        // decorator swaps it for the player markup (avoids FOUC).
                        $pending_voice_class = $is_voice_marker ? ' has-voice-pending' : '';
                        $message_html .= <<<HTML
                        <li class="message-bubble {$message_class_safe}{$related_class}{$pending_voice_class}">
