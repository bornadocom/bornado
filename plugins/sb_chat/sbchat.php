        ...
        public function SB_Chat_plugin_scripts() {
            ...
            wp_localize_script(
                ...
                array(
                    ...
                    'i18n' => array(
                        'cancelUpload'      => __( 'Cancel Upload', 'sb_chat' ),
                        /* translators: %s: error message from the file upload */
                        'uploadFailed'      => __( 'Upload failed: %s', 'sb_chat' ),
                        /* translators: %s: maximum number of files allowed in one upload */
                        'maxFilesExceeded'  => __( 'Only %s files can be uploaded at a time.', 'sb_chat' ),
                        'sending'           => __( 'Sending...', 'sb_chat' ),
                        /* translators: %s: pagination offset for the next batch */
                        'loadMore'          => __( 'Load more (%s)', 'sb_chat' ),
                    ),
                )
            );
...
//            wp_enqueue_script( 'dropzone-scri pt' );
        ...
        public function register_plugin_admin_scripts_styles( $hook ) {
            ...
            wp_localize_script( 'sbchat', 'sbchatLabels', array(
                'related_post_label' => __( 'Related post', 'sb_chat' ),
                'clearDataConfirm'   => __( 'Are you sure you want to clear plugin data?', 'sb_chat' ),
                'selectUser'         => __( '— Select User —', 'sb_chat' ),
                'securityIssue'      => __( 'Security issue, please verify your nonce', 'sb_chat' ),
            ) );
...
function add_custom_user_profile_fields($user) {
    ?>
    <h3><?php _e('Block User', 'sb_chat'); ?></h3>
    <table class="form-table">
        <tr>
            <th><label for="sb_is_user_blocked"><?php _e('Is User Blocked? This will block the user to send message', 'sb_chat'); ?></label></th>
            <td>
                <select name="sb_is_user_blocked" id="sb_is_user_blocked">
                    <option value="0" <?php selected(get_user_meta($user->ID, 'sb_is_user_blocked', true), 0); ?>><?php _e('No', 'sb_chat'); ?></option>
                    <option value="1" <?php selected(get_user_meta($user->ID, 'sb_is_user_blocked', true), 1); ?>><?php _e('Yes', 'sb_chat'); ?></option>
                </select>
            </td>
        </tr>
    </table>
